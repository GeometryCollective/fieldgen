...
namespace DDG
{
   class Quaternion
   {
     ...
     // yields z.re + i*z.im as a quaternion (j and k parts are therefore zero)

         Quaternion( const Vector& v );
         // initializes purely imaginary quaternion with specified imaginary (v) component

	 Quaternion( const Vector& e1, const Vector& e2, const Vector& e3 );
	 // gives the quaternion which rotates the standard frame
	 // (i,j,k) into the given frame orthonormal frame (e1,e2,e3)
	 // (the caller is responsible for making sure the frame is in
	 // fact orthonormal) assuming(!!!)  \tilde{v} = q v \bar{q}
	 // type rotation

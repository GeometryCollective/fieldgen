...
namespace DDG
{
   ...
   template <class T>
   void SparseMatrix<T> :: resize( int m_, int n_, int nnz )
   {
      m = m_;
      n = n_;
      ...
      lastCompressedColumn = 0;
      lastCompressedEntry = 0;
   }
   ...
   {
      // first check if the compressed matrix has been
      // built via other methods (like appendCompressedColumn())
      if( cData != NULL && data.size() == 0 )
      {
         return cData;
      }
   ...
   double residual( const SparseMatrix<T>& A,
                    ...
                    const  DenseMatrix<T>& x )
   // returns the max residual of the generalized eigenvalue problem
   // A x = lambda B x relative to the largest entry of the solution
   {
   ...
   template <class T>
   T rayleighQuotient( SparseMatrix<T>& A,
                       SparseMatrix<T>& B,
                        DenseMatrix<T>& x )
   // returns <Ax,x>/<Bx,x>
   {
      //return (x.transpose()*(A*x))(0) * (x.transpose()*(B*x))(0).inv();
      return (x.transpose()*(A.multiply(x)))(0) * (x.transpose()*(B.multiply(x)))(0).inv();
   }
